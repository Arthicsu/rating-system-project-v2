...
def rollover_semester(to_semester=None):
    ...
    2. обнулить живые поля баллов на всех студентах (история остаётся в SemesterScore);
    3. снять is_current с текущего семестра и поставить на `to_semester`
       (или на ближайший по start_date, если не задан).
    ...
    if current is not None:
        ...
        # Обнуляем живые поля (total_score пересчитается на уровне БД как GeneratedField).
        result['students_reset'] = Student.objects.update(
            academic_score=0, research_score=0, sport_score=0, social_score=0, cultural_score=0,
        )
