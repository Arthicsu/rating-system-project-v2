...
1. Склонируйте dev ветку:
   ```bash
   git clone -b dev --single-branch https://github.com/Arthicsu/rating-system-project-v2.git
   ```
---
...
- Да, компиляция страниц на клиенте очень медленная, так как используется старый сборщик webpack, поддерживающий watchPolling (перекомпиляцию клиентского кода после его изменения).
---
